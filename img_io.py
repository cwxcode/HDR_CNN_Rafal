...
def readLDR(file, sz, clip=True, sc=1.0):  # 读取8bit的LDR图片
    try:
...
def writeLDR(img, file, exposure=0):  # 保存8bit的LDR图片
...
def writeEXR(img, file):  # 保存HDR的exr图片
    try:
        img = np.squeeze(img)
        sz = img.shape
...
def load_training_pair(name_hdr, name_jpg):  # 读取训练的图片对
