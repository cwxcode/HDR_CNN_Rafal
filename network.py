...
# Weights are from a .npy模型文件 file generated with the caffe-tensorflow tool  采用caffe-tensorflow工具生成.npy模型文件
def load_vgg_weights(network, weight_file, session):
    params = []

    if weight_file.lower().endswith('.npy'):
        npy = np.load(weight_file, encoding='latin1')  # 加载预训练模型
