...
# Settings, using TensorFlow arguments  参数调节
...
tf.flags.DEFINE_float("gamma", "1.0", "Gamma/exponential curve applied before, and inverted after, prediction. This can be used to control the boost of reconstructed pixels.")  # gamma默认1.0
...
# If directory is supplied, get names of all files in the path  多张图片
...
load_params = tl.files.load_npz(name=FLAGS.params)  # 加载预训练模型
...
for i in range(len(frames)):
    ...
    try:
        # Read frame
        print_("\tReading...")
        x_buffer = img_io.readLDR(frames[i], (sy,sx), True, FLAGS.scaling)  # 读取图片
        print_("\tdone")

        print_("\t(Saturation: %0.2f%%)\n" % (100.0*(x_buffer>=1).sum()/x_buffer.size), 'm')  # 计算Saturation比率
        ...
        print_("\tInference...")
        feed_dict = {x: np.power(np.maximum(x_buffer, 0.0), 1.0/FLAGS.gamma)}  # 反gamma校正
        y_predict = sess.run([y], feed_dict=feed_dict)
        y_predict = np.power(np.maximum(y_predict, 0.0), FLAGS.gamma)
        print_("\tdone\n")

        # Gamma corrected output
        y_gamma = np.power(np.maximum(y_predict, 0.0), 0.5)  # gamma校正输出

        # Write to disc
        print_("\tWriting...")
        k += 1;
        # img_io.writeLDR(x_buffer, '%s/%06d_in.png' % (FLAGS.out_dir, k), 0)  # 将输入图片保存为png图片
        img_io.writeLDR(y_gamma, '%s/%06d_out.png' % (FLAGS.out_dir, k), 0)  # gamma校正输出的png图片
        img_io.writeEXR(y_predict, '%s/%06d_out.exr' % (FLAGS.out_dir, k))  # 没有gamma校正输出的exr图片
        print_("\tdone\n")
